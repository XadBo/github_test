...
class ASFF_EMA_2(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[64, 128]):
        super(ASFF_EMA_2, self).__init__()
        ...
        self.ema2 = EMA(inter_dim*2)
        self.conv_sample2 = Conv(self.inter_dim * 2, self.inter_dim, 3, 1)
    ...
    def forward(self, x):
        ...
        levels_weight_v = self.bincat2([input1, input2])
        fused_out_reduced = self.ema2(levels_weight_v)
        # fused_out_reduced = self.conv_sample2(levels_weight)
        # levels_weight = self.weight_levels(levels_weight_v)
        # levels_weight = nn.functional.softmax(levels_weight, dim=1)
        #
        # fused_out_reduced = input1 * levels_weight[:, 0:1, :, :] + \
        #                     input2 * levels_weight[:, 1:2, :, :]
        out = self.conv_sample2(fused_out_reduced)
        # out = self.conv(fused_out_reduced)

        return out


class ASFF_EMA_3(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[64, 128, 256]):
        super(ASFF_EMA_3, self).__init__()
        ...
        self.ema3 = EMA(inter_dim*3)
        self.conv_sample3 = Conv(self.inter_dim * 3, self.inter_dim, 3, 1)
    ...
    def forward(self, x):
        ...
        levels_weight_v = self.bincat3([input1, input2, input3])
        fused_out_reduced = self.ema3(levels_weight_v)
        # fused_out_reduced = self.conv_sample3(levels_weight)
        # levels_weight = self.weight_levels(levels_weight_v)
        # levels_weight = nn.functional.softmax(levels_weight, dim=1)
        #
        # fused_out_reduced = input1 * levels_weight[:, 0:1, :, :] + \
        ...
        out = self.conv_sample3(fused_out_reduced)
        # out = self.conv(fused_out_reduced)
